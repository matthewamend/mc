...
public:
  ...
  DeferStmt(Stmt *DeferredStmt, SourceLocation Loc);

  SourceLocation getBeginLoc() const { return this->DeferLoc; }
  SourceLocation getEndLoc() const { return this->DeferredStmt->getEndLoc(); }
  Stmt *getDeferredStmt() const { return this->DeferredStmt; }

  void setDeferLoc(SourceLocation Loc) { this->DeferLoc = Loc; }
  void setDeferredStmt(Stmt *S) { this->DeferredStmt = S; }

  static DeferStmt *CreateEmpty(const ASTContext &Context) {
    return new (Context) DeferStmt(EmptyShell());
  }

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == DeferStmtClass;
  }

  child_range children() {
    return child_range(&DeferredStmt, &DeferredStmt + 1);
  }
};
