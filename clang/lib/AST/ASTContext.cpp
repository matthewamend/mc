...
void ASTContext::CreateBuiltinTypedef(std::string Name, QualType Type) {
  TranslationUnitDecl *DC = getTranslationUnitDecl();
  IdentifierInfo *II = &Idents.get(Name);
  TypedefDecl *Decl =
      TypedefDecl::Create(*this, DC, SourceLocation(), SourceLocation(), II,
                          getTrivialTypeSourceInfo(Type, SourceLocation()));

  Decl->setImplicit(true);

  DC->addDecl(Decl);

  if (DeclContext *LookupDC = dyn_cast<DeclContext>(DC)) {
    LookupDC->makeDeclVisibleInContext(Decl);
  }
}

void ASTContext::InitMCTypes() {
  CreateBuiltinTypedef("i8", SignedCharTy);
  CreateBuiltinTypedef("i16", ShortTy);
  CreateBuiltinTypedef("i32", IntTy);
  CreateBuiltinTypedef("i64", LongLongTy);
  CreateBuiltinTypedef("i128", Int128Ty);

  CreateBuiltinTypedef("u8", UnsignedCharTy);
  CreateBuiltinTypedef("u16", UnsignedShortTy);
  CreateBuiltinTypedef("u32", UnsignedIntTy);
  CreateBuiltinTypedef("u64", UnsignedLongLongTy);
  CreateBuiltinTypedef("u128", UnsignedInt128Ty);

  CreateBuiltinTypedef("f16", Float16Ty);
  CreateBuiltinTypedef("f32", FloatTy);
  CreateBuiltinTypedef("f64", DoubleTy);
  CreateBuiltinTypedef("f128", Float128Ty);
}
...
bool ASTContext::compareStructFields(const RecordDecl *RDL,
                                     const RecordDecl *RDR) {
  assert(!RDL->isInvalidDecl() && !RDR->isInvalidDecl() &&
         "Bad declaration being compared?");
  size_t LHSNumFields =
      std::distance(RDL->fields().begin(), RDL->fields().end());
  size_t RHSNumFields =
      std::distance(RDR->fields().begin(), RDR->fields().end());

  if (LHSNumFields != RHSNumFields)
    return false;

  RecordDecl::field_iterator LI = RDL->field_begin();
  RecordDecl::field_iterator RI = RDR->field_begin();

  for (; LI != RDL->field_end() && RI != RDL->field_end(); LI++, RI++) {
    if (getTypeSize(&(*LI->getType())) != getTypeSize(&(*LI->getType())))
      return false;
  }
...
bool ASTContext::isIdenticalStructs(const RecordType *LHS,
                                    const RecordType *RHS) {
  const RecordDecl *RDL = LHS->getDecl();
  const RecordDecl *RDR = RHS->getDecl();

  if (RDL == RDR)
    return true;

  return compareStructFields(RDL, RDR);
}
...
bool ASTContext::typesAreCompatible(QualType LHS, QualType RHS,
  ...
  if (LHS->isStructureType() && RHS->isStructureType()) {
    return isIdenticalStructs(LHS->getAsStructureType(),
                                 RHS->getAsStructureType());
  }
