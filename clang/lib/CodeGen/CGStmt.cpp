...
void CodeGenFunction::EmitDeferStmt(const DeferStmt &S) {
  ...
  class DeferStmtCleanup final : public EHScopeStack::Cleanup {
    const Stmt *DeferredStmt;

  public:
    DeferStmtCleanup(const Stmt *Stmt) : DeferredStmt(Stmt) {}
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      CGF.EmitStmt(DeferredStmt);
    }
  };

  pushFullExprCleanup<DeferStmtCleanup>(NormalAndEHCleanup,
                                        S.getDeferredStmt());
}
