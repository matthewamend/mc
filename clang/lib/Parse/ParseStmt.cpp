...
StmtResult Parser::ParseDeferStatement() {
  ...
  StmtResult DeferredStmt;

  if (Tok.is(tok::l_brace)) {

    DeferredStmt = ParseCompoundStatement();
    if (DeferredStmt.isInvalid())
      return StmtError();
  } else {

    ExprResult DeferredExpr = ParseExpression();
    if (DeferredExpr.isInvalid())
      return StmtError();

    DeferredStmt = Actions.ActOnExprStmt(DeferredExpr.get());
  }

  return Actions.ActOnDeferStmt(DeferLoc, DeferredStmt.get());
}
